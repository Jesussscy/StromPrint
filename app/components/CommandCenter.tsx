...
const QUESTIONS: Question[] = [
  ...
  {
    id: "lluvia",
    label: "¿De dónde viene la lluvia?",
  ...
  {
    id: "riesgo",
    label: "¿Qué significan los colores?",
...
export default function CommandCenter() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "bot", text: "Hola, soy el asistente de STORM//PRINT. Seleccioná una pregunta para obtener información:" },
  ]);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="glass-strong mb-3 w-[360px] rounded-2xl overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-cyan/10 px-4 py-3">
              ...
              <button
                ...
                className="text-slate-500 hover:text-cyan transition font-mono text-[9px] uppercase"
              >
                Limpiar
              </button>
            </div>
                ...
                >
                  {QUESTIONS.map((q) => (
                    <motion.button
                      ...
                      className="glass rounded-lg px-2.5 py-2 text-left text-[11px] text-slate-300 hover:text-cyan hover:border-cyan/25 transition-all flex items-center gap-1.5"
                    >

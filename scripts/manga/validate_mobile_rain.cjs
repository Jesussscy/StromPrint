...
const { waterSurface } = require('../../app/lib/manga/waterSurface.ts');
const grid = { dx: 40, cells: [{ x: 0, y: 0, z: 0, area: 1600, built: 1, coastal: false, ij: [0, 0], triangles: [[[0, 0, 0], [2, 0, 2], [0, 2, 0]]] }], edges: [] };
...
for (let i = 0; i < surface.positions.length; i += 3) {
  assert.equal(surface.positions[i + 1], 1.015, 'horizontal cell water plane');
  assert.ok(surface.positions[i] <= 1, 'shoreline clipped at terrain crossing');
}
...
assert.deepEqual(response.result, result, 'cached terrain preserves forward integration');
